...
const GalleryAndVideo = () => {
  ...
  const renderVideoControls = useCallback(() => (
    <div
      className={`absolute inset-0 flex flex-col justify-between bg-transparent transition-opacity duration-300 ${showControls || !isPlaying ? 'opacity-100' : 'opacity-0'
        }`}
    >
      <div className="p-4">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <button
          ...
          className={`bg-white/80 p-4 rounded-full hover:bg-white transition-all transform hover:scale-110 pointer-events-auto ${showControls || !isPlaying ? 'opacity-100 scale-100' : 'opacity-0 scale-90'
            }`}
        >
          {isPlaying ? (
      ...
      <div className={`p-4 space-y-2 transition-transform duration-300 ${showControls ? 'translate-y-0' : 'translate-y-full'
        }`}>
                        ...
                        {index === activeIndex && (
                          ...
                          >
                            <button
                              ...
                              onMouseEnter={handlePlayButtonEnter}
                              onMouseLeave={handlePlayButtonLeave}
                              className={`bg-white/80 p-4 md:p-6 rounded-full hover:bg-white transition-all transform hover:scale-110 ${showControls || !isPlaying ? 'opacity-100 scale-100' : 'opacity-0 scale-90'
                                }`}
                            >
                              {isPlaying ? (
